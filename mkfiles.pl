...
sub mfval($) {
	...
	("vc","vcproj","cygwin","borland","lcc","devcppproj","gtk","unix",
         "ming", "am","osx","vstudio10","vstudio12")) {
        return 1;
    }
...
if (defined $makefiles{'ming'}) {
    $dirpfx = &dirpfx($makefiles{'ming'}, "/");

    ##-- Ming cross compile setup makefile, compiling for Windows from *nix
    open OUT, ">$makefiles{'ming'}"; select OUT;
    print
    "# Makefile for $project_name under Ming cross compiler.\n".
    ...
    "# by default it's just the prefix of the cross compiling tool in your PATH\n".
    "TOOLPATH ?= i686-w64-mingw32-\n".
    "CC = \$(TOOLPATH)gcc\n".
    "RC = \$(TOOLPATH)windres\n".
    "\n".
    &splitline("CFLAGS = -Wall -O2 -D_WINDOWS -DSECURITY_WIN32" .
		" -DWIN32S_COMPAT -D_NO_OLDNAMES -DBAD_IN6ADDR_MACROS " .
	       (join " ", map {"-I$dirpfx$_"} @srcdirs)) .
	       "\n".
    "LDFLAGS = -s\n".
    &splitline("RCFLAGS = \$(RCINC) --define WIN32=1 --define _WIN32=1".
		" --define WINVER=0x0400 -I..").
    "\n"."\n".
    ($makefile_extra{'ming'}->{'vars'}// '') .
    "\n".
    ".SUFFIXES:\n".
    "\n";
    ...
    foreach $d (&deps("X.o", "X.res.o", $dirpfx, "/", "ming")) {
